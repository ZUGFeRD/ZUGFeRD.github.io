...
image::./_images/image003.png[Slide © 2024 Andreas Pelekies validool.org]
...
The term changed to the plural, link:https://web.archive.org/web/20240831104010/https://www.ferd-net.de/aktuelles/meldungen/rueckblick-auf-die-sitzung-des-ferd-plenums.html[Zentrale User Guides Forum
...
The unit for Hectar is H18 square hectometre
...
Please note that a non-empty ApplicableHeaderTradeDelivery block remains required. The workaround is to duplicate the country ID of the recipient in ShipToTradeParty as follows:
...
=== How to identify specific stores as sale location
 German "Filialnummer/Niederlassungsnummer" : 

BT-46 , und/oder BT-46-0 (GLN – Rewe, Kaufland, Edeka etc.) or 

BT-71 , und oder BT-71-0 (GLN – Rewe, Kaufland, Edeka etc.)

=== How to specify vendor product categories
German "Warengruppe/Artikelgruppe"   - BT-158  -  Code „CG“

=== I dont have a VAT ID
You can still send electronic invoices using a ID without a scheme. This needs to be bilaterally agreed but if you put your tax ID and repeat it, there is some hope that your sender understands and agrees.

----
      <ram:SellerTradeParty>
        <ram:ID>12 123 123 123</ram:ID>
        ...
        <ram:SpecifiedTaxRegistration>
          <ram:ID schemeID="FC">12 123 123 123</ram:ID>
        </ram:SpecifiedTaxRegistration>
      </ram:SellerTradeParty>
----
However, this may not work long, a VAT ID may be required for a later CTC, so the recommendation is to request a VAT ID. Or at least a EBID-Number, link:https://unternehmensverzeichnis.org[unternehmensverzeichnis.org] assigns them for free (SchemeID 0189).
...
=== Where do I refer to the DeliveryNote

A reference to the delivery note (german "Lieferschein") is actually not foreseen in EN16931, it only supports Despatch Advice ("Lieferavis") in BT-16, i.e. DespatchAdviceReferencedDocument/IssuerAssignedID. Structured Delivery Notes are only possible in Factur-X/ZUGFeRD Extended with  BT-X-202 on document- (DeliveryNoteReferencedDocument/IssuerAssignedID) or with BT-X-92 (DeliveryNoteReferencedDocument/IssuerAssignedID) on line level.
...
The gross price in CII, UBL and EN16931 is without VAT, i.e. it is the net price without allowances and charges, more of a list price. Refer to e.g. link:https://xeinkauf.de/app/uploads/2024/07/302-XRechnung-2024-06-20.pdf[XRechnung spec] pg 95 The item price without VAT before allowances [because EN16931 knows no item charges] ("Der Postenpreis ohne Umsatzsteuer vor Abzug des Postenpreisrabatts").
